...
export default function Distribuidores() {
  ...
  const waMaquilaMsg = encodeURIComponent('Hola! Me interesa el servicio de maquila BIOS para mi cadena naturista. ¿Podrían darme información sobre disponibilidad y cotización?')
    ...
    <>
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .dist-programa-grid { grid-template-columns: 1fr; gap: 1rem; }
          .dist-maquila-grid { grid-template-columns: 1fr; gap: 1rem; }
          .dist-contacto-grid { grid-template-columns: 1fr; gap: 1rem; }
        }
      `}</style>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.25rem 1.5rem', background: 'rgba(247,243,236,0.93)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(61,43,26,0.08)' }}>
        ...
        <ul className="dist-nav-links">
          ...
          <li><a href="#maquila" style={{ fontSize: '0.8rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--bark-mid)', textDecoration: 'none' }}>Maquila</a></li>
      ...
      {/* MAQUILA BIOS */}
      <section id="maquila" style={{ background: 'var(--bark)', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,5rem)' }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2rem,4vw,3.8rem)', fontWeight: 300, lineHeight: 1.15, color: 'var(--cream)', marginBottom: '1rem' }}>
          Maquila BIOS para<br /><em style={{ color: 'var(--gold-light)' }}>cadenas naturistas.</em>
        </h2>
        <p style={{ fontSize: '1rem', fontWeight: 300, color: 'var(--stone-light)', lineHeight: 1.8, maxWidth: '600px', marginBottom: '1rem' }}>
          ¿Tu cadena naturista quiere ofrecer su propio vinagre de manzana con la calidad BIOS? Fabricamos tu vinagre sin conservadores, bajo un cupo limitado de producción mensual.
        </p>
        <div className="dist-maquila-grid">
          {[
            { title: 'Calidad BIOS garantizada', desc: 'Mismo estándar de elaboración que usamos en nuestros propios productos.' },
            { title: 'Sin conservadores', desc: 'Producción 100% natural, igual que el vinagre de manzana BIOS original.' },
            { title: 'Exclusivo para vinagre de manzana', desc: 'Este servicio de maquila aplica únicamente a vinagre de manzana (no incluye aceite de coco con orégano).' },
            { title: 'Cupos limitados', desc: 'Capacidad de producción restringida; se asignan por orden de solicitud y disponibilidad.' },
            { title: 'Pedido mínimo mensual: 350 litros', desc: 'Es el volumen mínimo requerido para apartar tu cupo de producción.' },
          ].map((b, i) => (
            <div key={i} style={{ padding: '2rem 1.5rem', border: '1px solid rgba(247,243,236,0.12)', borderRadius: '4px' }}>
              <p style={{ fontFamily: 'var(--font-serif)', fontSize: '1.15rem', fontWeight: 400, color: 'var(--cream)', marginBottom: '0.6rem' }}>{b.title}</p>
              <p style={{ fontSize: '0.85rem', fontWeight: 300, color: 'var(--stone-light)', lineHeight: 1.7 }}>{b.desc}</p>
            </div>
          ))}
        </div>
        <div style={{ marginTop: '3rem', padding: '2rem', background: 'rgba(247,243,236,0.06)', borderRadius: '4px', maxWidth: '600px' }}>
          <p style={{ fontSize: '0.95rem', color: 'var(--stone-light)', lineHeight: 1.8, marginBottom: '1.5rem' }}>
            ¿Tu cadena cumple con el mínimo mensual? Contáctanos para conocer disponibilidad de cupo y cotización.
          </p>
          <a href={`https://wa.me/${waNumber}?text=${waMaquilaMsg}`} target="_blank" rel="noopener noreferrer"
            style={{ display: 'inline-flex', alignItems: 'center', gap: '0.7rem', background: 'var(--gold)', color: 'var(--bark)', padding: '0.9rem 2rem', borderRadius: '3px', textDecoration: 'none', fontSize: '0.82rem', fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <div className="dist-contacto-grid">
            {[
              ...
              { label: 'Email', value: 'biosproductos@hotmail.com', icon: '✉️' },
          ...
          <a href={`https://wa.me/${waNumber}?text=${waMsg}`} target="_blank" rel="noopener noreferrer"
            style={{ display: 'inline-flex', alignItems: 'center', gap: '0.7rem', background: '#25D366', color: 'white', padding: '1rem 2rem', borderRadius: '3px', textDecoration: 'none', fontSize: '0.9rem', fontWeight: 500, marginTop: '2rem' }}>
